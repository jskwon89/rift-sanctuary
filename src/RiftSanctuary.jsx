...
- 경계의 눈 "지목당했다" → 누군가 그 사람을 대상으로 카드를 썼다는 뜻일 뿐이다. 정보 카드로 조사한 것일 수도 있고, 전투 카드일 수도 있다. 이것만으로는 아무도 의심할 근거가 안 된다.
- 추적자의 눈으로 누군가가 결계의 방패/정보 카드를 썼다는 것은 정상적인 행동이다. 이것만으로 의심하면 안 된다. 의심할 근거는 전투/방해 카드(칼날, 안개, 교란, 손길)를 썼을 때다.

★ 모순 감지 (반드시 지적하라):
- 같은 라운드에 이미 다른 사람이 주장한 카드를 또 썼다고 주장하면 → "같은 라운드에 같은 카드를 2명이 쓸 수 없다" 지적.
- 누군가 "PX가 A카드를 썼다"고 하는데 PX 본인이 "B카드를 썼다"고 했다면 → 둘 중 하나가 거짓말이므로 반드시 지적.
- 이전 라운드 발언과 모순되면 지적하라.

★ 의심의 근거:
- 의심하려면 반드시 논리적 인과관계가 있어야 한다. "어쩐지 수상하다"식의 근거 없는 의심은 하지 마라.
- 이전 라운드에 누군가를 "믿는다"고 했다면, 새로운 증거 없이 갑자기 의심하면 안 된다.`
    : `[공허 행동 규칙 — 중요!]
...
7. 누군가를 의심할 때 "P1이 의심스럽다"처럼 결론만 말하지 마라. 반드시 구체적 근거(카드 결과, 모순, 수상한 행동)를 함께 말해라.`;
}
